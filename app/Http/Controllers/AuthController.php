...
public function getUserFromPhone(Request $request){
    $user = User::where('mobile_phone', $request->phone)->first();
    if($user) {
        $userAccount = UserAccountDetail::where('user_id', $user->user_uid)->count();
...
public function getUserFromPhoneForBusiness($phone, Request $request){
...
public function updateUserAddress(Request $request) {
    ...
    if($validators->fails()) {
        ...
        if(isset($user)) {
            DB::table('users')->where('mobile_phone', $request->mobile_phone)->update(['address'=>$request->address]);
            return response()->json(['status'=>true, 'message'=>'address updated', 'address'=>$request->address]);
        } else {
...
public function generateEmailLink(Request $request)
{
      $validator = Validator::make($request->all(), [
        'uid' => 'required|exists:users,user_uid',
        'email' => 'required'
         ...
         if ($validator->fails()) {
            ...
            $url = 'https://web.noworri.com/auth/login/auth/forgot-password?'.$params;
...
public function verifyUser(Request $request)
    {
       ...
       if ($validator->fails()) {
            ...
            else return response()->json(['success' => 'SMS has been sent, please check your phone registered number '.$user_data['mobile_phone'], 'code'=>$user_data['code']]);
...
public function verifyUserPhone(Request $request)
    ...
    public function verifyUserPassword(Request $request)
    {
       ...
       if ($validator->fails()) {
        ...
        $pswd = bcrypt($request->password);
        echo $pswd;
        $user = DB::table('users')->where('mobile_phone', $request->mobile_phone)->where('password', $pswd)->first();
        if(isset($user)) {
    ...
    $validator = Validator::make($request->all(), [ 
       ...
       if ($validator->fails()) {
            ...
            if($user['otp'] == $request->otp) {
                ...
                return response()->json(["success" => true, "status" => true, "message" => "Email Verified successfully!"]);
            } else {
                return response()->json(["success" => false, "status" => false, "message" => "Invalid OTP or User"]);
            }
       }
}
...
public function sendEmailVerificationCode(Request $request){
       ...
       if ($validator->fails()) {
        ...
        $user = User::where('user_uid', $request->user_id)->orWhere('id',  $request->id)->orWhere('id',  $request->user_id)->first();
        $otp = $user['otp'];
        try {
                    if(isset($user))  {
    	            ...
    	            return response()->json(["success" => true, "status" => true, 'message'=>'Verification Code Sent']);
                } else {
                    return response()->json(["success" => false, "status" => false, 'message'=>'This user does not exist']);
                }

            }
            catch (Exception $e){
    		  echo "Error: " . $e->getMessage();
    		  return response()->json(["success" => false, "status" => false, 'message'=>$e->getMessage()]);
    	 }
    }
}
       ...
       if ($validator->fails()) {
        ...
        try {
                if(isset($user))  {
                    ...
                    $this->sendTermiiMessage($phoneNumber, $message);

	                return response()->json(["success" => true, "status" => true, 'message'=>'Verification Code Sent']);
                }  else {
                    return response()->json(['status'=>false, 'message'=>'This Phone number is not registered']);
                }
...
public function registerPalUser(Request $data) 
{
     // validate incoming request
        
        $validator = Validator::make($data->all(), [
            'email'      => 'required|email|unique:pal_users',
            'company_name'       => 'required|string|max:155',
            'full_name' => 'required|string|max:155',
            'phone_no'       => 'required|string|max:20|unique:pal_users',
            'password'   => 'string|min:6|max:255',
       ...
       if ($validator->fails()) {
          ...
          //  return response()->json(['error' => 'Unauthorized'], 401);
            ...
            $user = DB::table('pal_users')->insert($user_data);
            $userData = DB::table('pal_users')->where('email', $user_data['email'])->first();
            return response()->json(["status"=>true, "message"=>"User Registered", "data"=>$user]);
       }
}
       ...
       if ($validator->fails()) {
          ...
          //  return response()->json(['error' => 'Unauthorized'], 401);
            ...
            if(isset($user_data['address'])) {
                DB::table('users')->where('user_uid', $user_data['user_uid'])->update(['address'=>$user_data['address']]);
            }
            $details = [
                'subject' => 'Verify your email',
                'greeting' => 'Dear  ' . $user['first_name'],
...
public function deleteUserByPhoneNumber($mobile_phone) {
    $user=User::where('mobile_phone',$mobile_phone)->first();
    if($user) {
        $data = [
            'uid' => $user->user_uid,
            'identifient_email' => $user->email,
            'identifient_phone_number' => $user->mobile_phone,
            ];
        $res = DB::table('firebase_users_to_Delete')->insert($data);
        $user->delete();
        return response()->json(['status'=>true, 'message'=> 'User data deleted successfully']);
    } else {
        return response()->json(['status'=>false, 'message'=> 'User data deleted successfullyUser does not exists']);
    }
            ...
            return response()->json($validator->errors());
             ...
             if($request->code == $user['otp'])
             {
                ...
                if($dbCode && $dbCode->status === 'pending') {
                    $dbCode->update(['status' => 'verified']);
                } else {
                    return  response()->json(['status'=>false, 'result' => 'error', 'message' => 'OTP Invalid'], 402);
                }
               ...
               $message = "verified";
               return  response()->json(['status'=>true, 'result' => 'success', 'message' => $message, 'data'=>$user ], 200);
             }
             else
             {
               $message = "not verified";
               return  response()->json(['status'=>false, 'result' => 'error', 'message' => $message], 402);
             }
        }
...
private function checkUserPassword($suppliedPassword)
{
    return Hash::check($suppliedPassword, Auth::user()->password, []);
}

public function changeUserPassword(Request $requestData) {
    ...
    if($validator->fails()) {
        ...
        // $isValiveUser = $this->checkUserPassword($requestData->current_password);
        $fields = [
          'mobile_phone' => $requestData->mobile_phone,
          'password' => $requestData->current_password,
        ];
        $userVerification = $this->mockLogin($fields);
        $newPass = bcrypt($requestData->new_password);
        $response = json_decode($userVerification, true);
        if(isset ($response['currentUser'])) {
            User::where('mobile_phone', $requestData->mobile_phone)->update(['password' => $newPass]);
...
public function sendAccountCode(Request $request){
    ...
    $string = $this->generatePin();
        ...
        $sms_result = $this->sendCode($url, $opt);
        if($sms_result != null)  return response()->json(['error' => 'check your phone number', 'sms_error' => $sms_result ], 402);
        else return response()->json(['success' => 'SMS has been sent, please check your phone number '.$request->mobile_phone, 'code'=>$user_data['code']]);

}

private function mockLogin($fields) {
...
public function getCurrency($code)
{
   ...
   public function logintest()
   {
    ...
    $user = Auth::user();
    $userAccountTest = DB::table('user_account_details_test')->where('user_id', $user->user_uid)->count();
   ...
   public function login()
   {
    ...
    $user = Auth::user();
    $userAccountLive = UserAccountDetail::where('user_id', $user->user_uid)->count();
    $userAccount = $userAccountLive;
    $user->accounts = $userAccount;
    $currentUser = array_merge($user->toArray(), ['token' => $token, 'token_type' => 'bearer','expires_in' => auth()->factory()->getTTL() * 60 * 24 * 7 * 60]);
    return response()->json(compact('currentUser'));
    ...
    public function adminLogin(Request $request)
   {
    ...
    if ($user && $user->password === $pswd){
       ...
       $token = bin2hex($token);
       DB::table('admin_users')->where('email', $request->email)->update(['token'=>$token]);
        $data = [
   ...
   public function adminLogout() {
       $_SESSION['admin_user_token'] = null;
       ...
       if ($validator->fails()) {
        ...
        if ( isset($photo)) {
            ...
            if(file_exists($photoname)) {
                chmod($photoname,0755); //Change the file permissions if allowed
                unlink($photoname); //remove the file
            }
           ...
           // $photo->move(public_path().'/uploads/images/pp', $photoname);
            ...
            // ]);
           // return $user;
            return response()->json(['status'=>true, 'success'=>'File uploaded', 'file'=>$photoname]);
        }else{
            return response()->json(['status'=>false, 'error'=>'Filed cant be empty']);
        }
