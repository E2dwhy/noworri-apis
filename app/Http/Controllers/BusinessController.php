...
use FCM;
 ...
 define("MAXSIZE", 100);
    ...
    const BUSINESS_PENDING_STATUS = "pending";
    const BUSINESS_REJECTED_STATUS = "rejected";
...
class BusinessController extends Controller
{
    ...
    public function downloadPlugin()
    {
        ...
        $file= public_path(). "/checkout/woocommerce-gateway-noworri-escrow-for-woo-commerce.zip";
    
        $headers = array(
                  'Content-Type: application/zip',
                );
    
        return Response::download($file, 'woo-noworri.zip', $headers);
    }
       ...
       if ($validator->fails()) {
            ...
            if($businessData) {
                return response()->json(['message' => 'You already added a business']);
                 ...
                 $companyDocument = $request->file('company_document_path');
                if ( isset($logo)) {
                    ...
                    $logoFileName = $business_data['user_id'];
                    $uniqueIdLogoFileName = str_replace("/tmp/","",$logo);
                    $business_data['business_logo'] = $logoFileName.'/'.$uniqueIdLogoFileName;
                    
                    $resultLogoFile = $logo->move(public_path().'/uploads/company/business/'.$logoFileName);
                ...
                if ( isset($idCard)) {
                    $extansion = $idCard->getClientOriginalExtension();
                    // $idCardFileName = $idCard->getClientOriginalName();
                    $idCardFileName = $business_data['user_id'];
                    $uniqueIdCardFileName = str_replace("/tmp/","",$idCard);
                    $business_data['id_card'] = $idCardFileName.'/'.$uniqueIdCardFileName;
                    
                    $resultIdFile = $idCard->move(public_path().'/uploads/company/business/'.$idCardFileName);
                ...
                if(isset($companyDocument)) {
                    $extansion = $companyDocument->getClientOriginalExtension();
                    // $docFileName = $companyDocument->getClientOriginalName();
                    $docFileName = $business_data['user_id'];
                    $uniqueDocFileName = str_replace("/tmp/","",$companyDocument);
                    $business_data['company_document_path'] = $docFileName.'/'.$uniqueDocFileName;
                    $resultDocFile = $companyDocument->move(public_path().'/uploads/company/business/'.$docFileName);
                ...
                if(!isset($business_data['business_legal_name'])) {
                    $business_data['business_legal_name'] = 'N/A';
                }
                
                $business_data['status'] = BUSINESS_PENDING_STATUS;
                ...
                return response()->json(['status'=>true, 'data'=>$business]);
                    ...
                    return response()->json(['status'=>false, 'message'=>'somthing went wrong please try again']);
                }
            ...
            if ($validator->fails()) {
                return response()->json($validator->errors());
                    ...
                    if(!isset($fields['name'])) {
                         $fields['name'] = 'e-commerce';
                    }
                    $fields['transaction_source'] = TRANSACTION_SOURCE_ECOM;
                    $response = $this->storeNewBusinessTransactionTest($fields);
                    return response()->json(['status'=>true, 'message'=>'Transaction Created Successfully', 'data'=>$response]);
                }
    ...
    public function verifyUser(Request $request)
    {
        ...
        if(!$credentials) {
            ...
            $hasValidCred = $this->checkCredentials($credentials);
            if($hasValidCred['isValid'] === true) {
                $code = SmsVerification::where('code', $request->code)->first();
                if($code && $code->status === 'pending') {
                    DB::table('sms_verifications')->where('code', $request->code)->update(['status' => 'verified']);
                    return response()->json(['status' => 200, 'message' => 'User Verified']); 
                } else {
                    return response()->json(['status' => 401, 'message' => 'Invalid verification code']); 
                }
    ...
    public function sendVerificationCodeTest(Request $request)
    {
        ...
        if(!$credentials) {
           ...
           if ($validator->fails()) {
                ...
                SmsVerification::create(['code'=> $code, 'status'=>'pending', 'contact_number'=>$user_data['mobile_phone']]);
    ...
    public function getBusinessUserPayouts($user_id) {
        $payouts = DB::table('transfers')
        ->where('user_id', $user_id)
        ->get(['id','amount','bank_name','status','updated_at']);

        if ($payouts) {
            return response()->json(['status'=>true, 'processedPayouts'=>$payouts]);
        } else {
            return response()->json(['status'=>false, 'message' => 'User does not exist'], 404);
        }
    } 
    
    public function getBusinessUserPayoutsTest($user_id) {
       $payouts = DB::table('transfers')
        ->where('user_id', $user_id)
        ->get(['id','amount','bank_name','status','updated_at']);
